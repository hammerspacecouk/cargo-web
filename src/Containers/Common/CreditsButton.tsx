...
class CreditsButton extends React.Component<Props, LocalState> {
    ...
    isDisabled(amount: number, playerScore: ScoreInterface, disabledOverride: boolean): boolean {
        if (disabledOverride) {
            return true;
        }
        if (amount === 0) {
    ...
    update() {
        ...
        this.setState({
            disabled: this.isDisabled(this.props.amount, this.props.playerScore, this.props.disabled)
        });
...
export default connect(
    (state: StateInterface, ownProps: any): Props => ({
