...
class ShipNameContainer extends React.Component<Props, LocalState> {
    ...
    updateGuess() {
        ...
        if (!this.guessArray) {
            ...
            for (let i = 0; i < guessStartLength; i++) {
                this.guessArray.push(this.characters[Math.floor(Math.random()*this.characters.length)]);
            }
        }
        ...
        if (this.nameToMatch) {
            let namePadded = this.nameToMatch + new Array(guessLength).fill(' ').join('');
            let reduced = namePadded.slice(0, Math.max(this.nameToMatch.length, this.guessArray.length));
    ...
    renderOffer() {
        ...
        if (this.props.offeredShipNameToken &&
            this.state.matched
        ) {
            acceptanceButtons = (
    ...
    render() {
        ...
        if (this.props.requestingShipName ||
