...
export const SliderGroup: React.FC<IProps> = ({ className, sliders, maxTotal }) => {
  ...
  return (
    <div className={className}>
      <Header>
          <H3>Set new priorities <HelpButton
              type="button"
              onClick={(e) => {
                  e.preventDefault();
          ...
          >?</HelpButton></H3>
          <H4 as="span">Available: {remaining}/{maxTotal}</H4>
      </Header>
        {helpOpen && (
            <HelpText>
                <p>
                    Set the sliders for what is most important for you.
                    These can be changed at your next promotion.
                    You have {maxTotal} units to spread between:
                </p>
                <dl>
                    <dt>History</dt>
